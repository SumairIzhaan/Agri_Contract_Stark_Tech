...
const Flowers = () => {
    ...
    const fetchCrops = async () => {
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/crops?season=Flower`);
    ...
    if (loading) return <div className="text-center py-20 text-xl font-medium text-pink-600 animate-pulse">Loading Flowers...</div>;
    if (error) return <div className="text-center py-20 text-red-500 text-lg">Error: {error}</div>;

    return (
        <div className="container mx-auto px-4 py-12 pt-24 min-h-screen bg-pink-50/30">
            <div className="max-w-7xl mx-auto">
                <header className="text-center mb-16">
                    <h1 className="text-4xl md:text-5xl font-bold text-pink-800 mb-4 font-serif">Flowers</h1>
                    <p className="text-gray-600 max-w-2xl mx-auto text-lg leading-relaxed">
                        A guide to cultivating beautiful and commercially valuable flowers.
                    </p>
                </header>
                ...
                {crops.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {crops.map((crop) => (
                            <div key={crop.id} className="bg-white rounded-lg shadow-md p-4 border border-gray-200">
                                ...
                                <p className="text-sm text-pink-600 font-semibold mb-2">{crop.type}</p>
                    ...
                    <div className="text-center py-20 bg-white rounded-2xl shadow-sm border border-gray-100">
                        <div className="text-gray-400 text-xl font-medium">No flowers found.</div>

...
const Vegetables = () => {
    ...
    const fetchCrops = async () => {
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/crops?season=Vegetable`);
    ...
    if (loading) return <div className="text-center py-20 text-xl font-medium text-emerald-600 animate-pulse">Loading Vegetables...</div>;
    if (error) return <div className="text-center py-20 text-red-500 text-lg">Error: {error}</div>;

    return (
        <div className="container mx-auto px-4 py-12 pt-24 min-h-screen bg-emerald-50/30">
            <div className="max-w-7xl mx-auto">
                <header className="text-center mb-16">
                    <h1 className="text-4xl md:text-5xl font-bold text-emerald-800 mb-4 font-serif">Fresh Vegetables</h1>
                    <p className="text-gray-600 max-w-2xl mx-auto text-lg leading-relaxed">
                        Explore our guide to growing wide variety of fresh, nutritious vegetables in your garden or farm.
                    </p>
                </header>
                ...
                {crops.length > 0 ? (
                    ...
                    <div className="text-center py-20 bg-white rounded-2xl shadow-sm border border-gray-100">
                        <div className="text-gray-400 text-xl font-medium">No vegetables found.</div>

...
const Kharif = () => {
    ...
    const fetchCrops = async () => {
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/crops?season=Kharif`);
    ...
    if (loading) return <div className="text-center py-20 text-xl font-medium text-green-600 animate-pulse">Loading Kharif Crops...</div>;
    if (error) return <div className="text-center py-20 text-red-500 text-lg">Error: {error}</div>;

    return (
        <div className="container mx-auto px-4 py-12 pt-24 min-h-screen bg-green-50/30">
            <div className="max-w-7xl mx-auto">
                <header className="text-center mb-16">
                    <h1 className="text-4xl md:text-5xl font-bold text-green-800 mb-4 font-serif">Kharif Crops</h1>
                    <p className="text-gray-600 max-w-2xl mx-auto text-lg leading-relaxed">
                        Kharif crops, also known as monsoon crops, are domesticated plants that are cultivated and harvested in India during the monsoon season.
                    </p>
                </header>
                ...
                {crops.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {crops.map((crop) => (
                            <div key={crop.id} className="bg-white rounded-lg shadow-md p-4 border border-gray-200">
                                ...
                                <p className="text-sm text-green-600 font-semibold mb-2">{crop.type}</p>
                    ...
                    <div className="text-center py-20 bg-white rounded-2xl shadow-sm border border-gray-100">
                        <div className="text-gray-400 text-xl font-medium">No Kharif crops found.</div>
